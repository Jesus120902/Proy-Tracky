...
const Navbar = ({ toggleSidebar, user }) => (
  <header className="h-16 border-b border-secondary-200 bg-white/80 backdrop-blur-xl sticky top-0 z-30 px-6 flex items-center justify-between shadow-sm">
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="w-px h-6 bg-secondary-100" />

      <div className="flex items-center gap-3 pl-2 group">
        <div className="text-right hidden sm:block">
          <p className="text-sm font-black text-secondary-900 leading-none">{user.name}</p>
          <p className="text-[10px] uppercase tracking-wider text-primary-600 mt-1 font-black">
            {user.role}
          </p>
        </div>
...
const Layout = ({ children, user, onLogout }) => {
  ...
  return (
    <div
      className="min-h-screen bg-secondary-50 flex font-sans antialiased text-secondary-900"
      style={{ '--primary-600': primaryColor }}
    >
      {/* Estilos de animación del panel de notificaciones */}
      <NotificationStyles />
